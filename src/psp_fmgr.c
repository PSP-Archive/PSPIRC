...
int 
psp_file_request(char *out, char *pszStartPath, int up_down)
{
static  int sel=0;
...
static char loc_user_filename[PSP_FMGR_MAX_NAME];

char*
psp_fmgr_menu(char *user_file_dir, int up_down)
{
  struct stat  aStat;
  int          error;
  int          ret;
  ...
  if (psp_file_request(loc_user_filename, user_file_dir, up_down)) {
    error = 0;
    if (stat(loc_user_filename, &aStat)) error = 1;
    else 
    {
      error = 0;
    }
